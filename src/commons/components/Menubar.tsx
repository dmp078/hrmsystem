  ...
  return (
    <div className="h-full w-80 fixed top-11 bottom-0 pt-8 px-4 bg-white z-50">
      <div className="">
        <h4
          className={`${
            activePath.includes(ROUTES.home) && !activePath.includes(ROUTES.settings) ? "text-blue-500" : ""
          }`}
        >
        ...
        <div className="pl-6 mt-6 flex flex-col gap-4">
          {listGeneralHome.map((item, index) => (
            ...
            >
              <img className="w-8" src={require(`../../assets/images/icon${item}.png`)} alt="" />
              <div className={`my-auto ${activePath.includes(item) ? "text-blue-500" : ""}`}>
      ...
      <div className="mt-8">
        <h4 className={`${activePath.includes(ROUTES.settings) ? "text-blue-500" : ""}`}>
