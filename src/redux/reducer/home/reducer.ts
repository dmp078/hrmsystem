import { IemployeeState } from "../../../models/reducer/employeeReducer";
import { EMPLOYEE_ACTIONS } from "../../actions/home/actions";
...
export const reducerEmployee = (state: IemployeeState = initValue, action: IactionEmployee) => {
  switch (action.type) {
    ...
    case EMPLOYEE_ACTIONS.UPDATE_MARKED_DELETE:
      ...
      return {
        ...
        markedDelete: state.markedDelete?.includes(action.payload)
          ? state.markedDelete?.filter((item: any) => item != action.payload)
    ...
    case EMPLOYEE_ACTIONS.MULTI_MARKED_DELETE:
      ...
      return {
        ...
        markedDelete: [
          ...state.markedDelete,
          ...action.payload?.filter((item: any) => !state.markedDelete?.includes(item)),
        ],
      };
    ...
    case EMPLOYEE_ACTIONS.MULTI_CLEAR_MARKED_DELETE:
      ...
      return {
        ...state,
        markedDelete: [...state.markedDelete?.filter((item: any) => !action.payload?.includes(item))],
      };
